...
APP_VERSION = os.getenv("APP_VERSION", "2026-04-30-ada-only-v13")
...
- 只能根據「背景知識檢索」提供的 ADA 糖尿病指南片段回答。
- 不要使用模型內建知識、一般醫學常識、未載入指南、新聞或推測補完。
- 若指南片段沒有直接依據，請明確說目前指南知識庫資料不足，並停止回答。
- 回答中請自然標示依據來源，例如 ADA 2026；不要編造未出現在片段中的來源。
...
def build_retrieval_query(
    ...
    system_text = (
        "你不是回答者，也不要提供醫療建議。"
        "你的唯一任務是把 LINE 病友問題轉成 ADA 糖尿病指南文件檢索查詢。"
...
def select_guideline_hits(
    ...
    system_text = (
        ...
        "若 CKD/eGFR/腎臟問題有 ADA CKD、藥物選擇、eGFR 門檻相關候選，請優先保留它們；若片段不足以完整回答，answerable 必須是 false。"
